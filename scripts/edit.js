...
//			wiperAnimateCache(swiper); //隐藏动画元素 
	 ...
	 * 二级导航的显隐控制
	 */
	$(document).on('tap','.editable',function(ev){
		$('#subNav').toggle()
	})
	$(document).on('tap','#footer>li:not(.editable)',function(ev){
	 ...
	 * 保存场景编辑
	 */
	$(document).on('tap','#saveScene',function(ev){
		
		var btnArray = ['取消', '确定'];
        mui.confirm('保存编辑？', '信息提示', btnArray, function(e) {
            if (e.index == 1) {
            	//确定保存
            	localStorage.setItem("text1",$('.text1').text());
            	localStorage.setItem("text2",$('.text2').text());
            	// 消息表示
				mui.toast('保存成功', {
					duration: 'long',
					type: 'div'
...
//              alert(2)
	...
	$(document).on('tap','#previewScene',function(ev){
		//暂存修改的信息
		localStorage.setItem("text1",$('.text1').text());
    	localStorage.setItem("text2",$('.text2').text());
	 ...
	 * 删除当前页
	 */
	$(document).on('tap','.deletePage',function(ev){
		var btnArray = ['取消', '确定'];
...
//              alert(2)
	 ...
	 * 增加页
	 */
	$(document).on('tap','.addPage',function(ev){
		var pageIndex = $('.swiper-slide').index($('.swiper-slide-active'));
	 ...
	 *点击编辑文本
	 */
	$(document).on("tap", ".editText",function(event) {
	 ...
	 * 点击更换图片
	 */
	$(document).on("tap", ".editImg",function(event) {
	 ...
	 *确认编辑文本
	 */
	$(document).on("tap", "#ensure",function(event) {
